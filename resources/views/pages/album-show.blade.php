...
@section("content")
    ...
    <main class="min-h-[100vh] min-w-full py-28">
        ...
        <section class="mx-auto max-w-[96%] md:max-w-[90%] xl:max-w-[85%]">
            <section class="flex flex-col w-full">
                <section class="flex m-0 justify-between content-center">
                    ...
                    >
                        ...
                        >@lang("app.your_albums")</h1>
                        <p
                            class="text-md text-slate-600 mb-0"
                        >{{ count($albums) }} @lang('app.albums')</p>
                    </section>
                    @if(auth()->user()->Username == $user['Username'])
                        <button id="albumMenu" data-dropdown-toggle="dropdownMenu" class="p-0 m-0 text-lg text-inherit" type="button">
                        ...
                        <div id="dropdownMenu" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="albumMenu">
                                <li>
                                    <button data-modal-target="addAlbum" data-modal-toggle="addAlbum" class="w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" type="button">
                                        @lang('app.add_album')
                                    </button>
                                </li>
                            </ul>
            ...
            <section>
                @if (count($albums) > 0)
                    <section class="grid grid-cols-2 md:grid-cols-3 gap-4" role="list">
    ...
    <x-layouts.modal modalId="addAlbum">
        <x-slot:title>@lang('app.add_album')</x-slot:title>
        <form action="{{ route('album.store') }}" method="post" enctype="multipart/form-data" autocomplete="off"
            x-data="{
                inputCount: 1,
                inputLimit: 10,
        ...
        >
            ...
            <section class="flex flex-col gap-4">
                <section>
                    ...
                    <input type="text" id="album_title" name="namaalbum" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{ __('app.placeholder_album_title') }}" required />
                </section>
                <section>
            ...
            <button type="submit" class="text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center w-full mt-2">@lang('app.add')</button>
        </form>
    </x-layouts.modal>
