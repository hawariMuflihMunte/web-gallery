    ...
    <section class="flex">
        ...
        <template x-if="openList">
            <section class="relative">
                <section
                    class="absolute right-0 top-full rounded-sm border-slate-300 bg-white shadow min-w-[180px]"
                >
                    <ul class="flex flex-col">
                        <li>
                            <a
                                href="{{ route("profile.index") }}"
                                class="flex content-between align-center gap-2 bg-inherit p-3 text-slate-800 transition-all duration-100 hover:bg-emerald-400 w-full"
                            >
                                <i class="bi-person-circle"></i>
                                <span class="w-full ps-3 text-right">@lang('app.profile')</span>
                            </a>
                        </li>
                        <li>
                            <button
                                type="button"
                                class="flex content-between align-center gap-2 p-3 transition-all duration-100 hover:bg-red-500 hover:text-white w-full"
                                @click="logoutConfirm = true"
                            >
                                <i class="bi-x-square"></i>
                                <span class="w-full ps-3 text-right">@lang('app.logout')</span>
                            </button>
                        </li>
                    </ul>
                </section>
