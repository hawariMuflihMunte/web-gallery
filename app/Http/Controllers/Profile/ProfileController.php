...
class ProfileController extends Controller
{
  ...
  public function update(Request $request, string $id)
  {
      ...
      if (is_null($checkUpdate)) {
        ...
        if (!($update > 0)) {
          return redirect()
            ->back()
            ->withErrors([
              "error" => "Terjadi kesalahan, tidak dapat memperbarui data. Silahkan coba lagi !",
            ]);
        }
    ...
    if ($changes["username"] && $changes["email"] && $changes["namalengkap"]) {
      return redirect()
        ->back()
        ->with([
    ...
    if ($changes["namalengkap"]) {
      return redirect()
        ->back()
        ->with([
          "success-namalengkap" => "Berhasil memperbarui Nama Lengkap !",
        ]);
    }
